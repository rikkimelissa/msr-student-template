...
Using a multilayer perceptron and features describing the maximum velocity and position of each joint of the robot, we found an algorithm which increased our training accuracy by 7.41%. 

The data acquisition for this task was simplified by a script that calculated all parameters and path variables and recorded the relevant features. After a throw was executed, the operator recorded catch success. Each trial took approximately 30 seconds to run. This dataset was then used to predict whether a throw achieved catch success. 

Our machine learning solution performed better than expected. Our aim was to achieve 85% accuracy upon implementation on an algorithm. Key findings showed that features relating to the robot’s physical limits had a high correlation with catch success, and by focusing on these features we were able to increase our overall accuracy. Upon testing multiple algorithms (clustering, Bayes nets, and neural networks), we found that multilayer perceptrons performed with higher accuracy than the other two. With a pruned feature set and tuned parameters in the neural network, we achieved 85.71% accuracy on the training set through 10-fold cross validation. With this same model we achieved 87.5% accuracy on the set-aside test data, thereby exceeding our goal.
